...
# # Define the folder paths and global variables
# folder_path = os.path.join('Data', 'Debit', 'BofA')
# clean_folder_path = os.path.join('Data', 'Clean')
# bad_lines_log_path = os.path.join(clean_folder_path, 'bad_lines.txt')
# maps_folder_path = os.path.join('Cleaners', 'Maps')
...
# # Extract account and bank from the folder path
# account = os.path.basename(os.path.dirname(folder_path))
# bank = os.path.basename(folder_path)
...
#     if not clean_description:
#         keyword = input(f"KEYWORD:'{filename}' | '{amount}' | '{description}': ").lower()
#         clean_description = input(f"DSCRIPTION: '{keyword}': ")
#         description_mappings[keyword] = clean_description
#         with open(description_map_path, 'a') as file:
#             file.write(f"{keyword},{clean_description}\n")
...
#     category = category_mappings.get(clean_description, None)
#     if not category:
#         category = input(f"CATEGORY: '{clean_description}': ")
#         category_mappings[clean_description] = category
#         with open(category_map_path, 'a') as file:
#             file.write(f"{clean_description},{category}\n")
...
#             month_str = calendar.month_name[month_num]
#             clean_description, category = map_description(description, amount, os.path.basename(self.file), self.description_mappings, self.category_mappings)
#             final_row = [year, month_str, date, clean_description, category, amount, account, bank, "Savings"]
...
#             # Move and rename the cleaned file to Data > Clean folder
#             new_filename = f"{account}_{bank}_{filename}"
#             new_file_path = os.path.join(clean_folder_path, new_filename)
#             shutil.move(file, new_file_path)

#     print(f"{account} {bank} done")
...
class Cleaner:
    ...
    def clean_extra_headers(self):
        """Remove the first 6 lines from the CSV file."""
        with open(self.file, 'r') as csvfile:
            reader = csv.reader(csvfile)
            self.lines = list(reader)[6:]
    
    def log_bad_lines(self):
        """Log bad lines that do not have the correct number of columns."""
        cleaned_lines = []
        with open(bad_lines_log_path, 'a') as bad_lines_log:
            for row in self.lines:
                try:
    ...
    def remove_empty_amount(self):
        """Remove rows where the 'Amount' field is empty."""
        self.cleaned_lines = [row for row in self.cleaned_lines if row[2].strip()]
    
    def write_cleaned_file(self):
        """Write cleaned lines back to the CSV file."""
        with open(self.file, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerows(self.cleaned_lines)
    ...
    def clean(self):
        ...
        self.clean_extra_headers()
        self.log_bad_lines()
        self.remove_empty_amount()
        self.write_cleaned_file()
...
class Transformer:
    ...
    def remove_running_bal_column(self):
        """Remove the 'Running Bal.' column from the CSV file."""
        with open(self.file, 'r') as csvfile:
            reader = csv.reader(csvfile)
            headers = next(reader)
            running_bal_index = headers.index("Running Bal.")
            self.transformed_lines = [headers[:running_bal_index] + headers[running_bal_index+1:]]
            for row in reader:
                transformed_row = row[:running_bal_index] + row[running_bal_index+1:]
                self.transformed_lines.append(transformed_row)
    
    def map_description(self, description, amount, filename):
        """Map descriptions and categories based on mappings or prompt user for new mappings."""
        clean_description = None
        description_lower = description.lower()
        
        for original in self.description_mappings:
            if original in description_lower:
                clean_description = self.description_mappings[original]
                break
        
        if not clean_description:
            keyword = input(f"Enter keyword to record for '{description}' in file '{filename}' with amount '{amount}': ").lower()
            clean_description = input(f"Enter clean description for '{keyword}': ")
            self.description_mappings[keyword] = clean_description
            with open(description_map_path, 'a') as file:
                file.write(f"{keyword},{clean_description}\n")
        
        category = self.category_mappings.get(clean_description, None)
        if not category:
            category = input(f"Enter category for '{clean_description}': ")
            self.category_mappings[clean_description] = category
            with open(category_map_path, 'a') as file:
                file.write(f"{clean_description},{category}\n")
    ...
    def uniform_headers(self):
        """Transform the CSV to have uniform headers and map descriptions and categories."""
        uniform_headers = ["Year", "Month", "Date", "Description", "Category", "Amount", "Account", "Bank", "Card"]
        final_lines = [uniform_headers]
        for row in self.transformed_lines[1:]:
            date, description, amount = row[0], row[1], row[2]
            year, month_num, day = date.split('/')[2], int(date.split('/')[0]), date.split('/')[1]
            month_str = calendar.month_name[month_num]
            clean_description, category = self.map_description(description, amount, os.path.basename(self.file))
            final_row = [year, month_str, date, clean_description, category, amount, account, bank, "Savings"]
            final_lines.append(final_row)
        self.final_lines = final_lines
    
    def write_transformed_file(self):
        """Write transformed lines back to the CSV file."""
        with open(self.file, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerows(self.final_lines)
    
    def transform(self):
        """Perform all transformation steps."""
        self.remove_running_bal_column()
        self.uniform_headers()
